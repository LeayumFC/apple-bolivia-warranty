...
const emptyForm = {
  ...
  color: '',
  storage: '',
  condition: 'New' as Warranty['condition'],
  battery_health: 100,
  true_tone: false,
  face_id: false,
  touch_id: false,
  original_display: false,
  no_screen_scratches: false,
  no_body_dents: false,
  buttons_functional: false,
  cameras_functional: false,
  charging_port_functional: false,
  purchase_date: '',
  sale_date: '',
  customer_name: '',
  customer_email: '',
  warranty_months: undefined as number | undefined,
  notes: '',
};
...
const AdminWarranties = () => {
  ...
  useEffect(() => {
    if (authenticated) setWarranties(getWarranties());
  }, [authenticated]);
  ...
  const handleEdit = (w: Warranty) => {
    setForm({
      ...
      battery_health: w.battery_health || 100,
      true_tone: w.true_tone,
      face_id: w.face_id,
      touch_id: w.touch_id,
      original_display: w.original_display,
      no_screen_scratches: w.no_screen_scratches,
      no_body_dents: w.no_body_dents,
      buttons_functional: w.buttons_functional,
      cameras_functional: w.cameras_functional,
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <Card className="glass-card w-full max-w-sm">
          <CardHeader>
            <CardTitle className="text-center">Admin — Apple Bolivia</CardTitle>
          </CardHeader>
          <CardContent>
                ...
                {showChecklist && (
                  <Card className="border-border/50">
                    <CardHeader className="pb-2">
                      <CardTitle className="text-sm">Estado del Dispositivo</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {([
                          ['true_tone', 'True Tone funcional'],
                          ['face_id', 'Face ID funcional'],
                          ['touch_id', 'Touch ID funcional'],
                          ['original_display', 'Pantalla original'],
                          ['no_screen_scratches', 'Sin rayones en pantalla'],
                          ['no_body_dents', 'Sin abolladuras'],
                          ['buttons_functional', 'Botones funcionales'],
                          ['cameras_functional', 'Cámaras funcionales'],
                ...
                <div className="flex gap-2">
                  ...
                  {editingId && form.sale_date && form.customer_name && form.warranty_months && (
                    <Button
                      ...
                      variant="secondary"
                      onClick={() => {
                        const updated = updateWarranty(editingId, { ...form, status: 'Sold' });
                        setWarranties(getWarranties());
              ...
              <TableBody>
                {warranties.map(w => (
                  <TableRow key={w.id}>
                    ...
                    <TableCell>
                      <Badge variant={w.status === 'Sold' ? 'default' : 'outline'} className={w.status === 'Sold' ? 'bg-success text-success-foreground' : ''}>
                      ...
                      <div className="flex justify-end gap-1">
                        ...
                        {w.status === 'Unsold' && (
                          <Button variant="ghost" size="icon" onClick={() => handleMarkSold(w)} title="Vender">
                            <Badge className="text-xs bg-success text-success-foreground">$</Badge>
                          </Button>
                        )}
                        {w.status === 'Sold' && (
